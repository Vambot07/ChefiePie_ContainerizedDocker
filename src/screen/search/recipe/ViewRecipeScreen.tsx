...
const ViewRecipeScreen = () => {
    ...
    const handleSaveRecipe = async () => {
        ...
        setConfirmationModal({ visible: false, type: null }); // Close confirmation modal
    ...
    return (
        <View className="flex-1"
            ...
            {confirmationModal.type === 'save' && confirmationModal.itemToDelete && (
                <ConfirmationModal
                    visible={confirmationModal.visible}
                    onClose={() => setConfirmationModal({ visible: false, type: null })}
                    onConfirm={handleSaveRecipe}
                    title="Save Recipe"
                    message={`Would you like to save "${confirmationModal.itemToDelete.title}" for cooking?`}
                    confirmText="Save"
                    cancelText="Cancel"
                    icon="bookmark"
                    isDestructive={false}
            ...
            {confirmationModal.type === 'unsave' && confirmationModal.itemToDelete && (
                <ConfirmationModal
                    visible={confirmationModal.visible}
                    onClose={() => setConfirmationModal({ visible: false, type: null })}
                    onConfirm={handleUnsaveRecipe}
                    title="Unsave Recipe"
                    message={`Are you sure you want to unsave "${confirmationModal.itemToDelete.title}"?`}
                    confirmText="Unsave"
                    cancelText="Cancel"
